...
%macro COPY_ENV 2
	MALLOC r12, %1				; r12 new env
	mov r10, qword [rbp + 8*2]  ; r10 old env
...
%macro SHIFT_FRAME 2 
	...
	%rep %1 
		dec rax 
		mov r10, [rbp-WORD_BYTES*i]
		mov [rbp+WORD_BYTES*rax], r10
	%assign i i+1 
	%endrep
	pop r10
	pop rax
...
%macro MAKE_LIST 2
	...
	MAKE_PAIR(rdx , rdi ,SOB_NIL_ADDRESS)
	%%loop:
		cmp %2, 0
		je %%end
			mov rdi, [%1]
			MAKE_PAIR(r13 , rdi , rdx)
			mov rdx, r13
			sub %1, 8
		dec %2
		jmp %%loop
